...
<?php 
    ...
    if(!isset($_SESSION['admin_uid']) || !isset($_SESSION['login_date'])) {
        header("refresh:3;url=login.php?error=You need to be logged in to see this site");
        exit();
    }
    ...
    function getPaginatedAdminUsers($page, $perPage) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td>
            <a class='btn btn-outline-secondary' href='action.php?action=edit&source=admins&admin_uid=" . $admin_uid . "'>Edit</a>
            <a class='btn btn-outline-secondary btn-danger' href='action.php?action=delete&source=admins&admin_uid=" . $admin_uid . "'>Delete</a>
            </td>";
            echo "</tr>";
