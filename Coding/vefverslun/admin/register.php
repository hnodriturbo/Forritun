...
<!-- 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if($stmt = $conn->prepare("SELECT username FROM admin_accounts WHERE username = ?")) {
            ...
            if($result->num_rows > 0 ) {
                ...
                if($stmt = $conn->prepare("INSERT INTO `admin_accounts (`name`, `email`, `username`, `password`) VALUES (?, ?, ?, ?)")) {
                    $password = password_hash($_POST["password"], PASSWORD_DEFAULT);
                    $stmt->bind_param("sss", $_POST["name"], $_POST["email"], $_POST["username"], $password);
                    $stmt->execute();
